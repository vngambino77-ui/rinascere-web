export default async function handler(req, res) {
    ...
    try {
        ...
        if (!privateKey) return res.status(500).json({ error: 'Falta IMAGEKIT_PRIVATE_KEY' });
        ...
        if (!ikRes.ok) return res.status(500).json({ error: data.message || 'Error ImageKit' });
        return res.status(200).json({ url: data.url, fileId: data.fileId });
    } catch (error) {
